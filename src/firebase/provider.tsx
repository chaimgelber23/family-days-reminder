  ...
  useEffect(() => {
    ...
    const unsubscribe = onAuthStateChanged(
      ...
      (firebaseUser) => {
        ...
        // Set user state immediately for a responsive UI
        if (firebaseUser) {
          setUserAuthState({ user: firebaseUser, isUserLoading: false, userError: null });
          // Then, handle Firestore document in the background without blocking the UI update.
          ensureUserDocExists(firestore, firebaseUser).catch(err => {
              console.error('Failed to ensure user doc on auth state change:', err);
              // Optionally, update state to reflect this specific error
          });
        } else {
...
export function useMemoFirebase<T>(factory: () => T | null, deps: DependencyList): T | null {
  ...
  // This is a temporary workaround to avoid a bug in the studio that
  // causes infinite re-renders.
  try {
    (memoized as MemoFirebase<T>).__memo = true;
  } catch (e) {
    // some objects are frozen and can't be mutated.
  }
